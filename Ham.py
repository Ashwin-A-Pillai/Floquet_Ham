...
def eig_vec(params, F_modes, E_F, V_F,V,k_space, t):
    ...
    w_k = np.zeros((Nk,hdim), dtype=np.cdouble)
    ...
    for kp, vec_k in enumerate(k_space):
        ...
        for k, alpha in enumerate(orthnorm[:hdim]): #for k,o in enumerate(orthnorm):
           #print(f"{k=} and {o=}")
           chi_final[kp,k,:] = chi[kp,alpha,:]
           E_final[kp,k] = E_F[kp,alpha]
        ...
        if not np.allclose(S3, np.eye(hdim)):
           ...
           X[kp,:,:] = X[kp,:,:] / np.sqrt(S3[0,0])
           S = np.zeros_like(chi_mode[0, 0, 0, :])
           norm_w = 0.0
           for k,alpha in enumerate(orthnorm):
              w_k[kp,k]=np.vdot(X[kp, alpha, :],V[kp,:,k])
              norm_w = norm_w + abs(w_k[kp,k])**2
           print(f"{norm_w=}") 
           w_k[kp,:] = w_k[kp,k] / np.sqrt(norm_w)
        #print_formatted_matrix(I)
    #if not np.allclose(I[i,j], np.eye(Nm*hdim)):
    #    raise ValueError("The chi wavevectors are not orthogonal!")
    return E_final, chi_final, chi_mode, orthnorm, X ,w_k
...
def TD_wfn(params,F_modes,EF,VF,V,k_space,Tspc):
    ...
    for tp, t in enumerate(Tspc):
        E_final, chi_final, chi_mode, orthnorm, X ,w= eig_vec(params, F_modes, EF, VF,V,k_space, t)
        for kp, vec_k in enumerate(k_space):
          for k,alpha in enumerate(orthnorm):
            psi[tp,kp,:] = psi[tp,kp,:] + w[kp,k]*chi_final[kp,k,:]
          psi[tp,kp,:] = psi[tp,kp,:]/np.linalg.norm(psi[tp,kp,:])
    return E_final,psi,chi_mode, w, orthnorm